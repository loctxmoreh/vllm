...
def test_get_kv_cache_spec_kind_prefers_specific_attention_subclasses():
    ...
    assert (
        get_kv_cache_spec_kind(sliding_window_spec) == KVCacheSpecKind.SLIDING_WINDOW
    )
...
def test_get_kv_cache_spec_kind_unwraps_uniform_type_specs():
    ...
    uniform_swa_spec = UniformTypeKVCacheSpecs(
        block_size=16,
        kv_cache_specs={
    ...
    assert (
        get_kv_cache_spec_kind(uniform_swa_spec) == KVCacheSpecKind.SLIDING_WINDOW
    )
